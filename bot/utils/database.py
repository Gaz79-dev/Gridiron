...
class Database:
    ...
    async def create_event_archive(self, event_id: int, map_info: Dict[str, str] = None):
        ...
        async with self.pool.acquire() as connection:
            ...
            # A. Fetch all Squads for this event (Ensure we have every squad container)
            ...
            # Initialize: { "Squad Name": [] } - guarantees empty squads are empty lists, not nulls
            roster_snapshot = {row['name']: [] for row in squad_rows}
            squad_id_to_name = {row['squad_id']: row['name'] for row in squad_rows}

            # B. Fetch all Members (with names joined)
            ...
            # C. Stitch members into the dictionary
            nodes_snapshot = {}
    ...
    async def clear_event_message_id(self, event_id: int):
        query = "UPDATE events SET message_id = NULL WHERE event_id = $1;"
        async with self.pool.acquire() as connection:
            await connection.execute(query, event_id)
