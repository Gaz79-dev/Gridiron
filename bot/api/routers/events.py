...
async def promote_tentative_player(event_id: int, request: PromoteRequest, db: Database = Depends(get_db)):
    ...
    The roster refresh endpoint will handle adding them to the reserves squad.
    """
    ...
    try:
        # Update the player's RSVP status and role ONLY.
...
async def send_squad_embed(request: SendEmbedRequest, db: Database = Depends(get_db)):
    ...
    if request.squads:
        ...
        if first_squad: event_id = first_squad['event_id']
