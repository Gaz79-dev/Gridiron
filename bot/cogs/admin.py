...
class AdminCog(commands.Cog):
    ...
    async def sync_members(self, interaction: discord.Interaction):
        ...
        try:
            ...
            await interaction.followup.send(
                "Error: The bot is missing the 'Server Members Intent' or required permissions to fetch all members."
            )
        except Exception as e:
            print(f"An error occurred during member sync: {e}")
            await interaction.followup.send("An unexpected error occurred. Please check the bot's logs.")
